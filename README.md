...
- 已内置 DeepSeek 和通义千问两个模型预设，也支持自定义 OpenAI 兼容接口。
- 底部可以快速切换已保存的模型；每个会话会记住自己上次使用的模型。
- 设置里可以测试当前模型连接，方便先确认 API Key 和接口是否可用。
- 设置里可以复制诊断信息，便于内测反馈；不会包含 API Key 或聊天内容。
- 设置里可以复制内测反馈模板，适合朋友遇到问题时直接粘贴反馈。
- 设置里有内测发布小面板，发版前可快速确认版本、发布包和安全提醒。
- API Key 按模型分别保存在本机浏览器里，不会写入聊天备份。
- 联网轻量搜索仍是实验功能，在国内网络环境下可能偶尔失败，失败时会自动退回普通对话。
